...
public class _05_sort_LL {
    public static void main(String[] args) {
        ...
        static ListNode brute(ListNode head){
            ...
            ListNode ptr=head;
            while(ptr!=null){
                if(ptr.val==0) nZeroes++;
                else if(ptr.val==1) nOnes++;
                else if(ptr.val==2) nTwos++;
                ptr=ptr.next;
            }
            ptr=head;
        ...
        //--------------optimal: we just play with pointers in better instead of creating new ListNode, hence no space will be used
        static ListNode optimal(ListNode head){//O(N)---O(1)
            ...
            zero_tail.next = one_head.next != null ? one_head.next : two_head.next; // Connect 0s to 1s (or 2s if no 1s)
            two_tail.next = null; // Mark the end
