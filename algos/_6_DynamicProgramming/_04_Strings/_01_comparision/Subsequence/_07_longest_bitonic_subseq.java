...
/*Simulation: simple and easy

The idea: Just simulate the process of climbing the mountains. One first walk up, then after reaching peaks, one continues walking down until the valley. Then calculate the length that he/she has walked so far and start another climbing. Repeat the process and we get the answer. */
public int longestMountain(int[] arr) {
    int i=0,ans=0;
    while(i<arr.length){
         int base=i;
         while(i+1<arr.length && arr[i]<arr[i+1]) i++;//walking up
         if(i==base){//valid peak
             i++;
             continue;
         }
         int peak=i;
         while(i+1<arr.length && arr[i]>arr[i+1]) i++;//walking down
         if(i==peak){
             i++;
             continue;
         }
