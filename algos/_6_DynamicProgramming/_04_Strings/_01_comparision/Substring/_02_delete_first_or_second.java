...
//the purpose of adding this question is that though this question seems to be like dp, how it can be intelligentky solved, watch:https://www.youtube.com/watch?v=t7VVtb6Mu64&list=PLcXpkI9A-RZIpZs9Oq8tkuzc-_blEjCr1&index=2&ab_channel=TLEEliminators-byPriyansh, the first obs is that we dont need to perform 2nd opr separately coz doong 1st opr 2 times gives same sring as that obtained after the second operation, so like suppose i want the number of possibilities ie uniques strings that could be obtained in a string of length x, then observe that on performing oprs on original and reaching level where len is x in recursive tree, then last (x-1) chars for all x length strings would be same, ie just count number of unique chars till index i, sum them up and return

public class _02_delete_first_or_second{
    ...
    public static void solve() throws IOException {
        ...
        int nUnique[]=new int[len];
        boolean isSeen[]=new boolean[26];
        int idx=0;
        for(char c:s.toCharArray()){
            if(!isSeen[c-'a']){
                isSeen[c-'a']=true;
                nUnique[idx]+=(idx>0)?nUnique[idx-1]+1:1;
            }else{
                nUnique[idx]=nUnique[idx-1];
            }
            idx++;
        }
        //now i know nUnique chars from 0,i
        // System.out.println(Arrays.toString(nUnique));
        print(Arrays.stream(nUnique).sum());
    }
    ...
    static void rec(String s,Set<String>set){
        ...
        String temp=s.charAt(0)+((s.length()>=3)?(s.substring(2)):"");
        set.add(temp);
        rec(temp,set);
    }
