...
/*__________________________________GFG: arr has only pos integers
...
class Solution {
    ...
    public int minDifference(int nums[]) {
        ...
        for (int e : nums)
            total_sum += e;
        int dp[][] = new int[nums.length][total_sum];
        for (int b[] : dp)
            Arrays.fill(b, -1);
        return rec(nums, nums.length - 1, 0, dp);
    }

    private int total_sum;

    public int rec(int nums[], int i, int curr_sum, int dp[][]) {
        ...
        return dp[i][curr_sum] = Math.min(
                Math.min(
                        rec(nums, i - 1, curr_sum + nums[i], dp),
                        rec(nums, i - 1, curr_sum, dp)
                    ),
                Math.abs(total_sum - 2 * curr_sum)//abs diff betweenn sums of two partitions
                );
    }
}

/*
 * __________________________________LEETCODE: Arr can have both pos and neg
 * nums
...
public class _02_min_partition_difference {
    ...
    // --------------------recursive, instaed of using Empty lst and checking if
    // empty.size()==nums.length/2 which is very expensive, i used a variable called
    // nNumbersChosen thats all---TLE
    ...
    // ----------memoization: observe that we have 3 states changing, ie idx,currSum
    // and nNumbersChosen, BUT BUT BUT, if u use 3D dp ie
    // dp[idx][currrSum][nNumbersChosen], u may encounter error COZ currSum CAN BE
    // NEGATIVE, so instaed of that we will use HashMap<String,Integer> where key
    // will have format "idx|currSum|nNumbersChosen"---TLE
    public static int memoize(int nums[], int idx, int currSum, int sumOfAllElements, int nNumbsChosen,
    ...
    public static int tabulationWithNegNums(int nums[], int target, int totalSum) {
        ...
        for (int i = 0; i < nums.length; i++)
            dp.put(i + "|" + 0, true);
        if (nums[0] <= target)
            dp.put(0 + "|" + nums[0], true);
        for (int i = 1; i < nums.length; i++) {
            for (int currTar = 1; currTar <= target; currTar++) {
                String key = i - 1 + "";

                if (dp.get(key + "|" + (currTar - nums[i])) == null)
                    dp.put(key + "|" + (currTar - nums[i]), false);
                if (dp.get(key + "|" + currTar) == null)
                    dp.put(key + "|" + currTar, false);
    ...
    public static int optimal(int nums[]) {
        ...
        // All possible sum in left and right part (Generating and storing using
        // BIT-Masking)
        ...
        int res = Math.min(Math.abs(sum - 2 * left.get(N).get(0)), Math.abs(sum - 2 * right.get(N).get(0))); // To get
                                                                                                             // the
                                                                                                             // minimum
                                                                                                             // answer
                                                                                                             // from the
        ...
        for (int idx = 1; idx < N; ++idx) { // iterate from 1 so we dont have to include 0 and check for all value
                                            // except last as we have already considered it
            for (int a : left.get(idx)) { // check the sum at each number position
                ...
                if (pos < v.size()) {
                    res = Math.min(res, Math.abs(sum - 2 * (a + v.get(pos)))); // if found in vector then only update
                                                                               // otherwise continue
                }
            }
        }
