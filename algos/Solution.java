...
public class Solution{
    ...
    public static void solve() throws IOException {
        ...
        for(int i=0;i<len;i++) words[i]=scanString();
        print(longestCommonPrefix(words, len));
    }

    public static String longestCommonPrefix(String[] words, int n) throws IOException {
        ...
        outer:for(int i=0;i<words.length;i++){
           ...
           for(int j=1;j<=curr.length();j++){
               if(!set.contains(curr.substring(0,j))) continue outer;
           }
           if(curr.length()>=max){
               ans=curr;
           }
