...
public class _01_detect_cycle {
    ...
    // ------------------------------------------------------------------USING BFS
    // solution: if we find some vertex that [is not visited && already present in
    // the queue], there is a cycle
    // IMP watch:
    // https://www.youtube.com/watch?v=BPlrALf1LDU&list=PLgUwDviBIf0oE3gA41TKO2H5bHpPd7fzn&index=12&ab_channel=takeUforward

    public static boolean usingBFS_func(List<List<Integer>> adj) {// but why do we need this function additionally: what
                                                                  // if our graph has disconnected componts, and any one
                                                                  // of componts may heave cycles-------IMP
        int nNodes = adj.size();
    ...
    // -----------------------------my solution, used freq[nodeNumber] for each
    // node, incremented fre[nodeNumber] each time its encountered though its
    // visited already. Then if any of freq[nodeNumber]>=2 meaning it is neighbourer
    // of 2 nodes at a time even after being visited, there;s a cycle
    ...
    // -------------------------------------------------------------------------------DIRECTED
    // GRAPH:
    // Note that same code as that of undirectted wouldnt work coz in case of undir
    // graph we didnt hhave dirs and there could possibly be many ways, but here we
    // have directions, we can reach some nodes though it looks like a cycle in only
    // 1 way as we move in 1 dir
     ...
     * (0) ---> (1)
     * | ^
     * | | if we apply same algo here i will say there is cycle, but in actuall
     * there is no node from which if we start we can return at same point s
    ...
    // solution: to the same algo, we will store additionaly another array called
    // dfs_done array . whenever we encounter a node that is not visited, we mark it
    // visited , mark it as dfs_done , perform dfs on it, then after its dfs is done
    // mark it as dfs_not done again..during this process if we encounter any
    // neighbour for a node that is both visited and also dfs_done there exists a
    // cycle in it, so we return true
    ...
    public static boolean hasCycles_directed(ArrayList<ArrayList<Integer>> adj) {
        ...
        int parent[] = new int[nNodes];// for printing cycle
        Arrays.fill(parent, -1);
        for (int nodeNumber = 0; nodeNumber < nNodes; nodeNumber++) {
            if (!isVis[nodeNumber]) {
                boolean hasCycles = dfs_directed(adj, isVis, dfs_done, nodeNumber, parent);
                if (hasCycles)
                    return true;
            }
        }
    ...
    static boolean dfs_directed(ArrayList<ArrayList<Integer>> adj, boolean isVis[], boolean dfs_done[], int nodeNumber,
        ...
        for (int neighBour : adj.get(nodeNumber)) {
            if (!isVis[neighBour]) {
                boolean hasCycle = dfs_directed(adj, isVis, dfs_done, neighBour, parent);
                if (hasCycle)
                    return true;
            } else {
                // the node is visisted, check if its dfs is don or not
                if (dfs_done[neighBour]) {
                    // now there is cycle etween nodeNumber and neighBour
                    // ----------------------printing cycle:
                    List<Integer> cycle = new ArrayList<>();
                    cycle.add(nodeNumber);
                    while (nodeNumber!=neighBour) {
    ...
    // the below uses kahn's algo as described in _12_top_sort, since it works only
    // for DAG(Directed acyclic graph), if we try to apply for a cyclic graph, we
    // will arrive at a point where the q becomes empty without goin through all
    // nodes, ie fails to add all nodes topologically
